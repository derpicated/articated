    ...
    if (const_buffer.isValid ()) {
        ...
        switch (const_buffer.handleType ()) {
            ...
            case QVideoFrame::HandleType::RhiTextureHandle: {
                ...
                QVideoFrameFormat::PixelFormat format = const_buffer.pixelFormat ();

                if (format == QVideoFrameFormat::PixelFormat::Format_XBGR8888 ||
                format == QVideoFrameFormat::PixelFormat::Format_XRGB8888) {
                    size_t pixelsize;
                    if (format == QVideoFrameFormat::PixelFormat::Format_XBGR8888) {
                        pixelsize    = 4;
                        image.format = BGR32;
                    ...
                    auto tex_name = const_buffer.textureHandle (0);
                    if (output_to_texture) {
                        output_texture = tex_name;
                        // background_is_grayscale_ = false;
                    }
                    DownloadImage (image, tex_name);
                } else {
                }
                break;
            }
