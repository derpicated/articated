...
template float Classification::sum<float> (std::vector<float> values);
template double Classification::sum<double> (std::vector<double> values);
...
float Classification::scale (const points_t& reference_points,
...
translation_t Classification::translation (const points_t& reference_points,
const points_t& data_points) {
...
float Classification::yaw (const points_t& reference_points, const points_t& data_points) {
    // rotation from top (Z)
    /* match points */
    points_t ref  = reference_points;
    points_t data = data_points;
...
float Classification::pitch (const points_t& reference_points, const points_t& data_points) {
    // y/row
    /* match points */
    points_t ref  = reference_points;
    points_t data = data_points;
...
float Classification::roll (const points_t& reference_points, const points_t& data_points) {
    // x/coll
    /* match points */
    points_t ref  = reference_points;
    points_t data = data_points;
...
void Classification::translate (points_t& points, translation_t T) {
    if (T.x == 0 && T.y == 0) {
        return;
...
void Classification::scale (points_t& points, const float scale) {
    if (!points.size ()) {
        return;
...
float Classification::dot_product (const point_t& p1, const point_t& p2) {
...
float Classification::dot_product_degrees (const point_t& p1, const point_t& p2) {
    return dot_product (p1, p2) * (180 / M_PI);
}

float Classification::projected_angle_abs (const float R, const float D) {
...
void Classification::find_matching_intersection (const points_t& reference_points,
const points_t& data_points,
const line_t& line,
float& ref_length,
float& data_length) {
...
point_t Classification::centroid (const points_t& points) {
...
template <typename T> T Classification::sum (std::vector<T> values) {
...
point_t Classification::sum (const points_t& points) {
...
float Classification::closest (const std::vector<float>& vec, float compare) {
    if (vec.empty ()) {
        return 0;
    }
...
bool Classification::is_in_front (line_t L, point_t R, point_t P) {
...
bool Classification::equal (float a, float b, float error) {
...
bool Classification::almost_equal_rel_abs (float A, float B, float maxDiff, float maxRelDiff) {
    // Check if the numbers are really close -- needed
    // when comparing numbers near zero.
...
point_t Classification::intersections (point_t p1, point_t p2, point_t origin) {
...
point_t Classification::intersection (line_t l1, line_t l2) {
    point_t I;
...
float Classification::a (line_t line) {
...
float Classification::b (line_t line) {
    float A = a (line);
    // horizontal line,
    // A == 0
    if (equal (A, POINT_ZERO)) {
...
float Classification::x (float y, line_t line) {
    float A = a (line);
    float B = b (line);
...
float Classification::y (float x, line_t line) {
    float A = a (line);
    float B = b (line);
    if (equal (A, POINT_ZERO) && B != POINT_INF) {
...
float Classification::distance (point_t A, point_t B) {
    point_t d = { std::fabs (B.x - A.x), std::fabs (B.y - A.y) };
    return std::sqrt (d.x * d.x + d.y * d.y);
}

void Classification::match_points (const points_t& reference_points, points_t& data_points) {
...
template <typename T> std::string Classification::to_string (T value) {
